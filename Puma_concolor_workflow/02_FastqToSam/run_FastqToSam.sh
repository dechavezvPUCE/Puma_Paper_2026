#! /bin/bash

#SBATCH -N 1  # number of nodes
#SBATCH -n 1  # number of "tasks" (default: allocates 1 core per task)
#SBATCH -t 0-34:00   # time in d-hh:mm:ss
#SBATCH -p public # partition
#SBATCH -q public	# QOS
#SBATCH -o /data/CEM/bottleneck/dechave4/Puma/log/FastqToSAM.Pumas.out # file to save job's STDOUT (%j = JobId)
#SBATCH -e /data/CEM/bottleneck/dechave4/Puma/log/FastqToSAM.Pumas.err # file to save job's STDERR (%j = JobId)
#SBATCH --mail-type=ALL # Send an e-mail when a job starts, stops, or fails
#SBATCH --mail-user=dechave4@asu.edu # Mail-to address
#SBATCH --export=NONE   # Purge the job-submitting shell environment
#SBATCH --mem=10000
# USAGE: qsub ./run_FastqToSam.sh [dir] [read_1] [read_2] [outfile] [RG] [sample] [library] [flowcell] [seq center]

module load java/8u102

PICARD=/data/CEM/bottleneck/programs/picard-tools/current/picard.jar

DIR=${1}
cd ${DIR}

mkdir /data/CEM/bottleneck/dechave4/Pumas/temp/${2%.*}
TEMP_DIR=/data/CEM/bottleneck/dechave4/Pumas/temp/${2%.*}

java -Xmx18G -jar -Djava.io.tmpdir=${TEMP_DIR} ${PICARD} FastqToSam \
FASTQ=${DIR}/${2} \
FASTQ2=${DIR}/${3} \
OUTPUT=${DIR}/${4} \
READ_GROUP_NAME=${5} \
SAMPLE_NAME=${6} \
LIBRARY_NAME=${7} \
PLATFORM_UNIT=${8} \
PLATFORM=illumina \
SEQUENCING_CENTER=${9} \
TMP_DIR=${TEMP_DIR}
