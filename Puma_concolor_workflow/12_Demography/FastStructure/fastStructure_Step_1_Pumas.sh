...
calldate=20251222 # date that genotypes were called
indir=/u/scratch/d/dechavez/Puma/data/VCF/Puma/Combined/onlyPass
# temporary
...
infile=Joint_Puma_ALLchr_Annot_Mask_Filter_passingSNPs.vcf
outdir=/data/CEM/bottleneck/dechave4/Puma/data/VCF/Puma/Combined/UCLA-Puma/Trimmed/onlyPass/SNPRelate2025/fastStructure
list=No.realted.Indv.txt
mkdir -p $outdir
...
#plink --remove $indir/${list} --vcf $indir/$infile --make-bed --keep-allele-order --const-fid 0 -chr-set 38 no-x no-y no-xy no-mt -maf 0.1 -out $outdir/${infile%.vcf.gz}
plink --remove $indir/${list} --vcf $indir/$infile --make-bed --keep-allele-order --const-fid 0 --allow-extra-chr -maf 0.1 -out $outdir/${infile%.vcf.gz}
### --remove $indir/${list}

#/u/home/d/dechavez/plink2
#--allow-extra-chr
#--keep-allele-order
