...
calldate=20240903 # date gt's were called in format YYYYMMDD
todaysdate=format(Sys.Date(),format="%Y%m%d")
# file locations:
pops=c("Brazil","CayambeCoca","Mashpi","Arenillas")
indir="/u/scratch/d/dechavez/Puma/VCF/Puma/Combined/onlyPass" # this is where your snp vcf file is and where you will save your gds file (downloaded from hoffman)
plotoutdir=paste("/u/scratch/d/dechavez/Puma/VCF/Puma/Combined/onlyPass/",calldate,"/Relatedness/plots",sep="")
fileoutdir=paste("/u/scratch/d/dechavez/Puma/VCF/Puma/Combined/onlyPass/",calldate,"/Relatedness/results",sep="")
dir.create(plotoutdir,recursive = T)
dir.create(fileoutdir,recursive = T)

#open the gds file
genofile <- snpgdsOpen(paste(indir,"/Joint_Puma_chrAll_Annot_Mask_Filter_passingSNPs_removeInds_pruned.gds",sep=""))
...
popmap = read.table("/u/scratch/d/dechavez/Puma/VCF/Puma/Combined/onlyPass/SA.list.Puma.for.PCA.txt",header=T)
head(popmap)
sample.id = as.character(popmap$Sample)
pop1_code = as.character(popmap$PrimaryPop)
pop2_code = as.character(popmap$PrimaryPop)

# separate by population and make sure the samples are in dataset (removed some low cov ind)
Brazil.id <- popmap[popmap$PrimaryPop == "Brazil" & popmap$Sample %in% samp.id,]$Sample
CayambeCoca.id <- popmap[popmap$PrimaryPop == "CayambeCoca" & popmap$Sample %in% samp.id,]$Sample
Mashpi.id <- popmap[popmap$PrimaryPop == "Mashpi" & popmap$Sample %in% samp.id,]$Sample
Arenillas.id <- popmap[popmap$PrimaryPop == "Arenillas" & popmap$Sample %in% samp.id,]$Sample
...
Brazil.ibd <- snpgdsIBDMoM(genofile, sample.id=Brazil.id, snp.id=snpset.id, remove.monosnp = T,
                    maf=0.05, missing.rate=0.05, num.thread=2,autosome.only = F)
CayambeCoca.ibd <- snpgdsIBDMoM(genofile, sample.id=CayambeCoca.id, snp.id=snpset.id,remove.monosnp = T,
                       maf=0.05, missing.rate=0.05, num.thread=2,autosome.only = F)
Mashpi.ibd <- snpgdsIBDMoM(genofile, sample.id=Mashpi.id, snp.id=snpset.id,remove.monosnp = T,
                       maf=0.05, missing.rate=0.05, num.thread=2,autosome.only = F)
Arenillas.ibd <- snpgdsIBDMoM(genofile, sample.id=Arenillas.id, snp.id=snpset.id,remove.monosnp = T,
                       maf=0.05, missing.rate=0.05, num.thread=2,autosome.only = F)

Brazil.ibd.coeff <- snpgdsIBDSelection(Brazil.ibd)
Brazil.ibd.coeff$population <- "Brazil"
CayambeCoca.ibd.coeff <- snpgdsIBDSelection(CayambeCoca.ibd)
CayambeCoca.ibd.coeff$population <- "CayambeCoca"
Mashpi.ibd.coeff <- snpgdsIBDSelection(Mashpi.ibd)
Mashpi.ibd.coeff$population <- "Mashpi"
Arenillas.ibd.coeff <- snpgdsIBDSelection(Arenillas.ibd)
Arenillas.ibd.coeff$population <- "Arenillas"

# put into one dataframe
all.ibd.coeff <- rbind(Brazil.ibd.coeff,CayambeCoca.ibd.coeff,
Mashpi.ibd.coeff)
#print(all.ibd.coeff)
# color column
all.ibd.coeff$KinshipGreaterThan0.2 <- "no"
all.ibd.coeff[all.ibd.coeff$kinship > 0.2,]$KinshipGreaterThan0.2 <- "yes"
all.ibd.coeff$pop1 <- all.ibd.coeff$ID1
all.ibd.coeff$pop2 <-  all.ibd.coeff$ID2
print(all.ibd.coeff)
# fix BER/MED to COM
...
pops=c("Brazil","CayambeCoca","Mashpi")
