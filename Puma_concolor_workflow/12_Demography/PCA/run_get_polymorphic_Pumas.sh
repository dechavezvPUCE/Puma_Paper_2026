#! /bin/bash
#$ -wd /u/scratch/d/dechavez/Puma/VCF/Puma/Combined
#$ -l h_rt=2:00:00,h_data=5G,arch=intel*
#$ -N OnlySNPs
#$ -o /u/scratch/d/dechavez/Puma/log/OnlySNP.out
#$ -e /u/scratch/d/dechavez/Puma/log/OnlySNP.err
#$ -m abe
#$ -M dechavezv
...
Direc=/data/CEM/bottleneck/dechave4/Puma/data/VCF/Puma/UCLA-Puma/Combined/Trimmed

cd ${Direc}

#mkdir -p ${Direc}/onlyPass
##PREFIX=$1
...
#INFILE=Joint_chr${i}.Puma.TrimAlt_Annot_Mask_Filter.vcf.gz
INFILE=$1
OUTFILE=${INFILE%.vcf.gz}_passingSNPs.vcf.gz


zcat ${INFILE} | head -2300 | grep "^#" > ${INFILE}_head
...
cat ${INFILE}_head - | ${BGZIP} > ${Direc}/onlyPass/${OUTFILE}
